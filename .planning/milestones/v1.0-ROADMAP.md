...
**Version:** v1.0  
**Name:** Copland Overnight Hardening  
**Status:** ✅ SHIPPED 2026-04-03  
**Phases:** 1-13  
**Total Plans:** 23  

This file preserves the full live roadmap as it stood when v1.0 shipped.
...
- [x] **Phase 6: Multi-Repo Runner** - Add repos: list to global config and make copland run iterate all repos sequentially
- [x] **Phase 7: Launchd Setup** - Add copland setup command that installs a macOS launchd plist for nightly automation
- [x] **Phase 8: Retry Wrapper Tests** - Pest tests for AnthropicApiClient covering retry logic, backoff, and non-retryable errors
- [x] **Phase 9: Executor Tests** - Pest tests for ClaudeExecutorService covering tool dispatch, thrashing, and policy violations
- [x] **Phase 10: Orchestrator Tests** - Pest tests for RunOrchestratorService covering all 8 pipeline steps and early-exit paths
- [x] **Phase 11: Documentation** - Rewrite README for Copland and add overnight setup guide
- [x] **Phase 12: Multi-Repo Failure Logging** - Ensure repo failures caught before orchestrator startup still append structured run-log entries
- [x] **Phase 13: Verification Backfill** - Add missing verification artifacts for Phases 4-11 and re-close milestone audit gaps
...
**Plans**: 2
**Plan list**:
- [x] 06-01-PLAN.md — GlobalConfig Updates and RunCommand Refactoring
- [x] 06-02-PLAN.md — Multi-Repo Iteration and Execution
...
**Plans**: 2
**Plan list**:
- [x] 07-01-PLAN.md — SetupCommand and LaunchAgent plist generation
- [x] 07-02-PLAN.md — launchctl reload flow and macOS verification
**UI hint**: no
...
**Plans**: 1
**Plan list**:
- [x] 08-01-PLAN.md — AnthropicApiClient retry and backoff tests
...
**Plans**: 1
**Plan list**:
- [x] 09-01-PLAN.md — ClaudeExecutorService service-level executor tests
...
**Plans**: 1
**Plan list**:
- [x] 10-01-PLAN.md — RunOrchestratorService service-level orchestrator tests
...
**Plans**: 1
**Plan list**:
- [x] 11-01-PLAN.md — README rewrite and overnight setup guide

### Phase 12: Multi-Repo Failure Logging
**Goal**: Repo-level failures caught before `RunOrchestratorService` starts still produce structured JSONL run records
**Depends on**: Phase 6 (multi-repo loop exists), Phase 3 (run log exists)
**Requirements**: OBS-01, SCHED-02
**Gap Closure**: Closes milestone audit integration gap for pre-orchestrator repo failures bypassing `RunLogStore`
**Success Criteria** (what must be TRUE):
  1. A repo failure raised before `RunOrchestratorService` starts still appends a structured run log entry under `~/.copland/logs/runs.jsonl`
  2. The logged record includes repo, failed status, timestamps, and failure reason for the pre-orchestrator path
  3. Multi-repo `copland run` continues to the next repo after such a failure
  4. Regression coverage locks this path so `OBS-01` and `SCHED-02` stay satisfied together
**Plans**: 1
**Plan list**:
- [x] 12-01-PLAN.md — Pre-orchestrator failure logging and regression coverage

### Phase 13: Verification Backfill
**Goal**: Milestone verification coverage is complete for Phases 4-11 so shipped requirements are no longer orphaned from audit evidence
**Depends on**: Phase 12 (real integration defect should be closed before audit backfill)
**Requirements**: COST-01, COST-02, SCHED-01, SCHED-03, TEST-03, TEST-01, TEST-02, DOCS-01, DOCS-02
**Gap Closure**: Closes milestone audit requirement orphans caused by missing `VERIFICATION.md` artifacts for Phases 4-11
**Success Criteria** (what must be TRUE):
  1. Phases 4-11 each have a verification artifact with status, evidence, and requirement coverage
  2. Re-auditing the milestone no longer reports orphaned requirements for Phases 4-11
  3. Verification backfill does not overstate behavior that is not actually implemented
  4. Any real defects surfaced during backfill are fixed or explicitly re-opened as separate gap work
**Plans**: 1
**Plan list**:
- [x] 13-01-PLAN.md — Verification artifact backfill and milestone audit refresh
...
| 6. Multi-Repo Runner | 2/2 | Complete | 2026-04-03 |
| 7. Launchd Setup | 2/2 | Complete | 2026-04-03 |
| 8. Retry Wrapper Tests | 1/1 | Complete | 2026-04-03 |
| 9. Executor Tests | 1/1 | Complete | 2026-04-03 |
| 10. Orchestrator Tests | 1/1 | Complete | 2026-04-03 |
| 11. Documentation | 1/1 | Complete | 2026-04-03 |
| 12. Multi-Repo Failure Logging | 1/1 | Complete | 2026-04-03 |
| 13. Verification Backfill | 1/1 | Complete | 2026-04-03 |
