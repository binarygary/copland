# Requirements Archive: v1.1 Multi-Provider & Asana Integration
...
- [x] **PROV-03**: User can set the default LLM provider in `~/.copland.yml`
- [x] **PROV-04**: User can override the LLM provider per repo in `.copland.yml`
- [x] **PROV-05**: User can configure different providers for selector, planner, and executor stages independently

### Ollama (OLLAMA)

- [x] **OLLAMA-01**: User can configure Ollama as a provider with base URL and model name
- [x] **OLLAMA-02**: Copland probes Ollama reachability before starting the orchestration loop and fails fast with a clear error if unreachable
- [x] **OLLAMA-03**: Copland warns at startup if the configured Ollama model is known to have poor tool-use support

### OpenRouter (OPENR)

- [x] **OPENR-01**: User can configure OpenRouter as a provider with API key and model name
- [x] **OPENR-02**: Copland sends attribution headers (`HTTP-Referer`, `X-Title`) on OpenRouter requests

### Asana (ASANA)

- [x] **ASANA-01**: User can map Asana projects to repos in `~/.copland.yml`
- [x] **ASANA-02**: Copland fetches open tasks from a configured Asana project (same selection pipeline as GitHub Issues)
- [x] **ASANA-03**: User can filter which Asana tasks Copland picks up by tag or section name (configured in `~/.copland.yml`)
...
| OLLAMA-01 | Phase 15 | Complete |
| OLLAMA-02 | Phase 15 | Complete |
| OLLAMA-03 | Phase 15 | Complete |
| OPENR-01 | Phase 15 | Complete |
| OPENR-02 | Phase 15 | Complete |
| ASANA-01 | Phase 17 | Complete |
| ASANA-02 | Phase 17 | Complete |
| ASANA-03 | Phase 17 | Complete |
